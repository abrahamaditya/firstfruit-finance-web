...
const formStorageKeys = (
  ...
  const form = `${descriptor.type}:${descriptor.id ?? descriptor.sourceSavingId ?? 'new'}:${descriptor.duplicate ? 'copy' : 'normal'}`;
  return {
...
export function useMoney() {
  ...
  return {
    ...
    // Formatter dasar sengaja absolut karena mayoritas komponen memasang arah (+/−)
    // sendiri. Nilai hasil perhitungan seperti arus kas bebas membutuhkan minus bawaan.
    fmtSigned: (idr: number) =>
      `${idr < 0 ? '−' : ''}${formatMoney(Math.abs(idr), ui.prefs.currency, ui.rate)}`,
    fmtCompactSigned: (idr: number) =>
      `${idr < 0 ? '−' : ''}${formatMoneyCompact(Math.abs(idr), ui.prefs.currency, ui.rate)}`,
  };
}
...
interface FieldDefinition {
  ...
  placeholder?: string;
  optional?: boolean;
  requiredIf?: (form: Record<string, string>) => boolean;
...
const FORM_SECTIONS: Record<CreateType, FormSectionDefinition[]> = {
  ...
  budget: [
    { title: 'Informasi anggaran', keys: ['name'] },
    { title: 'Nilai anggaran', keys: ['allocated'] },
  ],
  periode: [
  ...
  ambil: [{ title: 'Dana yang ditarik', keys: ['amount'] }],
  reminder: [
...
const EXPENSE_PILLAR_OPTIONS: CategoryOption[] = [
  { value: 'Needs', label: 'Needs — Kebutuhan Pokok' },
  { value: 'Wants', label: 'Wants — Keinginan & Gaya Hidup' },
  { value: 'Giving', label: 'Giving — Persembahan & Taburan' },
  ...
  if (path.length <= 1) {
    ...
    return path.length === 1
      ? {
          ...
          subCategory: '',
          categoryDetail: '',
          incomeCategory: ['Needs', 'Wants', 'Giving', 'Savings', 'Receivables'].includes(path[0]) ? '' : path[0],
          incomePillar: '',
...
const applyFieldChange = (form: Record<string, string>, key: string, value: string) => {
  ...
  if (key === 'toWalletId') {
    return { ...form, toWalletId: value, savingId: value === form.walletId ? '' : 'none' };
  }
  if (key === 'medium') {
    return {
      ...form,
  ...
  if (key === 'bank') {
    return { ...form, bank: value, pasporVariant: '' };
  }
...
function Inner({ initialPreferences }: { initialPreferences?: Preferences }) {
  ...
  useLayoutEffect(() => {
    ...
    setOpenSuggest(null);
    const openSheet = appRef.current?.querySelector<HTMLElement>('.sheet.show');
    openSheet?.scrollTo({ top: 0, left: 0, behavior: 'auto' });
    if (sheet === 'create') {
  ...
  useEffect(() => {
    ...
    void Promise.all([
      ...
      const tools = Array.isArray(row?.home_tools)
        ...
        : DEFAULT_HOME_TOOLS;
      const walletOrder = Array.isArray(row?.wallet_order)
        ? row.wallet_order.filter((id: unknown): id is string => typeof id === 'string')
  ...
  useEffect(() => {
    ...
    const timer = window.setTimeout(() => {
      void getBrowserSupabase().from('user_workspace_preferences').upsert({
        ...
        wallet_order: prefs.walletOrder,
  ...
  const openSavingTransfer = useCallback((savingId: string, savingName: string) => {
    void repos.savings.get(savingId)
      .then((saving) => {
        if (!saving || saving.archived) {
          notify('Tabungan tidak tersedia');
          return;
        }
        setFormDraftReady(false);
        ...
        setCreate({
          ...
          sourceWalletId: saving.walletId,
        });
        setSheet('create');
      })
      .catch((caught) => {
        notify(caught instanceof Error ? caught.message : 'Gagal membuka transfer tabungan');
      });
  }, [notify, repos]);
  ...
  const formConfig = useCallback(
    (type: CreateType): FormConfig => {
      ...
      const isCreditPayment = (form: Record<string, string>) =>
        isTransfer(form)
      ...
      const transaksiConfig: FormConfig = {
        ...
        fields: [
          ...
          },
          {
            ...
            optional: true,
            optionsOf: (f) => [
              ...(f.walletId === f.toWalletId
                ? []
                : [{ value: 'none', label: 'Tidak dimasukkan ke tabungan' }]),
              ...(savingsFor(f.toWalletId).length > 0
                ? savingsFor(f.toWalletId)
                : [{ value: '', label: 'Belum ada tabungan di dompet ini' }]),
            ],
            requiredIf: (f) => isTransfer(f) && f.walletId === f.toWalletId,
            showIf: (f) =>
              isTransfer(f)
              && (f.walletId === f.toWalletId || savingsFor(f.toWalletId).length > 0),
          },
          {
            key: 'isInstallment',
            label: 'Apakah transaksi ini cicilan?',
            type: 'segmented',
            options: [
              { value: 'no', label: 'Tidak' },
              { value: 'yes', label: 'Ya' },
            ],
            showIf: usesCreditCard,
          },
          {
            key: 'installmentTenor',
          ...
          },
          {
            ...
            type: 'select',
            options: EXPENSE_PILLAR_OPTIONS,
            showIf: isExpense,
          },
          {
            key: 'debtor',
          ...
          },
          {
            ...
            showIf: (f) => isExpense(f) && Boolean(f.pillar) && f.pillar !== 'Receivables',
          },
          {
            key: 'categoryDetail',
          ...
          },
          {
            ...
            type: 'select',
            options: budgetOptions.length > 0
              ? budgetOptions
              : [{ value: '', label: 'Belum ada anggaran di periode ini' }],
            showIf: (f) => isBudgetable(f) && f.includeBudget === 'yes',
          },
          {
            key: 'note',
            label: 'Catatan',
      ...
      const configs: Record<CreateType, FormConfig> = {
        wallet: {
          ...
          fields: [
            ...
            },
            {
              ...
              showIf: (f) => f.medium === 'bank' && f.bank === 'BCA',
            },
            {
              key: 'bank',
              label: 'Produk dompet',
              type: 'select',
              optionsOf: (f) => walletProductsFor(f.medium).map((product) => ({
                value: product.value,
                label: product.label,
              })),
              showIf: (f) => Boolean(f.medium) && f.medium !== 'cash',
            },
            {
              key: 'balance',
            ...
            },
            {
              ...
              showIf: (f) => f.medium === 'bank' || f.medium === 'digital' || f.medium === 'credit',
            },
        ...
        planning: {
          ...
          defaults: { title: '', target: '', saved: '', targetDate: '', status: '' },
        },
        piutang: {
          title: 'piutang',
          ...
          defaults: { person: '', amount: '', source: '', date: '' },
        },
        budget: {
          title: 'anggaran',
        ...
        tabungan: {
          title: 'tabungan',
          description: 'Buat dana yang disisihkan untuk diri sendiri atau orang lain. Saldo awal selalu Rp0 dan dananya ditambahkan melalui transaksi Transfer.',
          fields: [
            { key: 'emoji', label: 'Ikon (emoji)', placeholder: '🎓' },
            { key: 'name', label: 'Nama tabungan', placeholder: 'Masukkan nama tabungan' },
            {
              key: 'ownership',
              label: 'Dana ini untuk',
              type: 'segmented',
              options: [
        ...
        ambil: {
          title: 'tarik',
          description: 'Tarik sebagian tabungan ke saldo tersedia.',
          fields: [{ key: 'amount', label: 'Jumlah ditarik', type: 'number' }],
          defaults: { amount: '' },
        },
        reminder: {
          title: 'pengingat',
  ...
  useEffect(() => {
    ...
    Promise.all([repos.budgets.list(), repos.transactions.list(), repos.periods.list()])
      ...
      setBudgetOptions(
        activePeriod
          ? budgets
              .filter((budget) => budget.periodId === activePeriod.id)
              .sort((a, b) => a.category.localeCompare(
                b.category,
                prefs.language === 'EN' ? 'en-US' : 'id-ID',
                { sensitivity: 'base' },
              ))
              .map((budget) => ({ value: budget.id, label: budget.category }))
          : [],
      );
  ...
  }, [repos, dataVersion, prefs.language]);
  ...
  useEffect(() => {
    ...
    try {
      ...
      setCreate({
        ...
        sourceSavingId: typeof saved.sourceSavingId === 'string' ? saved.sourceSavingId : undefined,
        sourceWalletId: typeof saved.sourceWalletId === 'string' ? saved.sourceWalletId : undefined,
      });
      setSheet('create');
    } catch {
  ...
  useEffect(() => {
    ...
    const presetDate = create.type === 'reminder' && !create.id && /^\d{4}-\d{2}-\d{2}$/.test(create.name || '')
      ...
      : null;
    const presetSavingTransfer =
      create.type === 'transfer' && create.sourceSavingId && create.sourceWalletId
        ? {
            txType: 'transfer',
            walletId: create.sourceWalletId,
            toWalletId: create.sourceWalletId,
            savingId: create.sourceSavingId,
          }
        : null;
    ...
    const load = async () => {
      ...
      const loaded = Object.fromEntries(
        config.fields
          ...
          // Nilai default bertahan untuk record lama yang belum punya field terkait.
  ...
  const saveForm = async (event: FormEvent) => {
    ...
    if (
      ...
      && form.walletId === form.toWalletId
      && (!form.savingId || form.savingId === 'none')
    ) {
    ...
    try {
      if (create.type === 'wallet') {
        const category = form.medium || 'bank';
        ...
        const payload = {
          name: medium === 'cash'
            ? 'Uang Tunai'
            : form.bank === 'BCA' && form.pasporVariant
              ? `Paspor BCA ${form.pasporVariant}`
              : product?.label ?? form.bank.trim(),
      ...
      if (create.type === 'transaksi' || create.type === 'transfer') {
        ...
        const isCreditPayment = isTransfer
          && walletOptions.some((wallet) => wallet.value === form.toWalletId && wallet.kind === 'credit');
        const usesCreditCard = type === 'expense'
          && walletOptions.some((wallet) => wallet.value === form.walletId && wallet.kind === 'credit');
      ...
      if (create.type === 'sisihkan' || create.type === 'ambil') {
        ...
        if (create.type === 'sisihkan') {
          ...
          if (amount > saving.balance) {
            notify(`Maksimal ${formatIDR(saving.balance)} bisa ditarik`);
            return;
          }
      ...
      if (create.type === 'budget') {
        const payload = {
          ...
          // Realisasi selalu berasal dari transaksi; repository tidak pernah menyimpan
          // nilai ini ke tabel budgets.
          spent: 0,
        };
      ...
      notify(
        create.type === 'orang'
          ...
          : create.type === 'sisihkan'
            ...
            : create.type === 'ambil'
              ? `${savedAmount} ditarik dari ${create.name ?? 'tabungan'}`
  ...
  const transactionDetailsToggle = hasTransactionDetails ? (
    ...
    >
      <span>
        ...
        <small>Catatan, tempat, atau anggaran</small>
      </span>
      <Chevron />
    </button>
  ) : null;
        ...
        <section className={`sheet${sheet === 'item' ? ' show' : ''}`} aria-label="Tindakan item">
          ...
          {item.type === 'tabungan' && (
            <>
              <button
                className="act"
                onClick={() => item.id && openSavingTransfer(item.id, item.name)}
              >
                <span className="ax"><Plus /></span> {t('common.add')}
              </button>
              <button className="act" onClick={() => openCreate('ambil', false, item.name, item.id)}>
                <span className="ax"><ArrowUp /></span> {t('wallets.take')}
              </button>
            </>
          )}
        ...
        <section className={`sheet form-sheet${sheet === 'create' ? ' show' : ''}`} aria-label="Form data">
          ...
          <h3>
            {create.type === 'sisihkan'
              ...
              : create.type === 'ambil'
                ? `Tarik dari ${create.name ?? 'tabungan'}`
                  ...
                  : MONEY_FIELDS.has(field.key) ? (
                    ...
                    >
                      ...
                      {field.type === 'date' && !form[field.key] && (
                        <span className="date-field-value date-field-placeholder" aria-hidden="true">
                          {field.placeholder || t('common.chooseDate')}
                        </span>
                      )}
                      {field.type === 'date' && form[field.key] && (
